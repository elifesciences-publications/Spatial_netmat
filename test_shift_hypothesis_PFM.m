...
map_thresh = 0; % Set to 1 for a fixed threshold and to -95 for a percentile threshold
map_bin = 0; % Set to 1 to threshold and binarize subject maps
...
drTC = zeros(size(ts.ts,1),size(ts.ts,2));
drTC_ORIG = zeros(size(ts.ts,1),size(ts.ts,2));
...
if map_thresh < 0
    save(sprintf('Results/results_PFM%s.mat',A),'OUTPUT','F','P','-v7.3','Lthr', 'Uthr')
else
    save(sprintf('Results/results_PFM%s.mat',A),'OUTPUT','F','P','-v7.3')
end    
save(sprintf('Results/input_CCA_PFM%s.mat',A),'FccaNEW','FccaORIG','PccaNEW','PccaORIG')
