...
Yeo = ft_read_cifti('/vols/Scratch/janineb/HCP/netmat_simulations/Results/Yeo_ROIs_final.dtseries.nii');
...
drTC_NEW = zeros(size(ts.ts,1),size(Yeo,2));
drTC_ORIG = zeros(size(ts.ts,1),size(Yeo,2));
...
for s = 1:ts.Nsubjects
    ...
    for x = 1:4;
        ...
        msk_tmp = [msk_tmp; nets_demean(D' * Yeo)];
    ...
    drTC_NEW((s-1)*ts.NtimepointsPerSubject+1:s*ts.NtimepointsPerSubject,:) = msk_tmp;
    drTC_ORIG((s-1)*ts.NtimepointsPerSubject+1:s*ts.NtimepointsPerSubject,:) = msk_tmp2;
end
...
tsNEW = ts; tsNEW.Nnodes = size(Yeo,2);
...
P.netNEW = nets_netmats(tsNEW,-1,'ridgep',rho);
...
if map_thresh < 0
    save(sprintf('Results/results_Yeo%s.mat',A),'OUTPUT','F','P','-v7.3','Lthr', 'Uthr')
else
    save(sprintf('Results/results_Yeo%s.mat',A),'OUTPUT','F','P','-v7.3')
end 
save(sprintf('Results/input_CCA_Yeo%s.mat',A),'FccaNEW','FccaORIG','PccaNEW','PccaORIG')   
